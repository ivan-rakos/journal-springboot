...
public class AccountControllerTest {
    ...
    void getAccountByIdNotFound() throws Exception {
        when(accountService.getById(1L)).thenThrow(new ResourceNotFoundException("Account with id 1 not found"));
    ...
    void updateAccountSuccess() throws Exception {
        var updateAccountDTO = TestDataFactory.createValidUpdateAccountDTO();
        var updatedAccount = TestDataFactory.updatedAccount();
    ...
    void updateAccountEmptyName() throws Exception {
        ...
        mockMvc.perform(patch("/api/accounts/1")
                ...
                .content(objectMapper.writeValueAsString(updateAccountDTO)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error.name").value("Name can only contain letters, numbers, and spaces"));
    }
    ...
    void deleteAccountNotFound() throws Exception {
        doThrow(new ResourceNotFoundException("Account with id 1 not found")).when(accountService).deleteAccount(1L);
